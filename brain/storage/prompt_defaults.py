...
{context}""",
    ...
    (
        "query_rewriter",
        "查询改写器",
        "Multi-Query 生成多个语义等价但用词不同的查询版本，提升知识库召回率（Phase 5F FR60）",
        """你是搜索查询改写专家。给定一个搜索查询，生成 {count} 个语义等价但用词不同的版本，用于提升知识库召回率。

要求：
- 保持原查询的核心意图不变
- 用不同的同义词、表达方式、句式改写
- 每行一个改写版本，不要编号、不要解释、不要额外说明
- 改写后的查询应能检索到与原查询相同的知识
...
改写版本（每行一个）：""",
    ...
    (
        "daily_digest",
        "每日知识简报",
        "基于昨日摄入的笔记生成知识简报（含占位符模板）",
        """你是一个个人知识管家。基于用户昨日摄入的笔记，生成一份知识简报。
...
保持简洁，总字数控制在 300 字以内。""",
    ...
    (
        "weekly_trend",
        "每周知识趋势",
        "基于本周笔记摄入分析知识趋势（含占位符模板）",
        """你是一个个人知识管家。基于用户本周的笔记摄入，分析知识趋势。
...
总字数控制在 400 字以内。""",
    ...
    (
        "cli_ask",
        "CLI 简单问答",
        "CLI brain ask 命令的简单问答提示词（不走 DeepAgents，含占位符模板）",
        """你是一个个人知识库助手。基于用户的知识库内容回答问题。
